...
int DepthFirstSearch(int start, int end) {
...
int DLS(int start, int end, int depth) {
...
int IterativeDepthLimitedSearch (int start, int end) {
	for(int depth = 0; ; depth++) {
		int found = DLS(start, end, depth);
		if(found != -1){
			return found;
		}
	}
...
int main(int argc, char** argv)
{
	...
	int foundD = DepthFirstSearch(start,end);
	int foundB = BreadthFirstSearch(start,end);
	int foundI = IterativeDepthLimitedSearch(start,end);
